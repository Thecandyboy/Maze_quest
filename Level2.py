...
def play_level2(player,WINDOW):
    ...
    boosters = []
    for j in range(NUMBER_OF_BOOSTERS_2):
    ...
    while running :
        ...
        if space_ability_used and pygame.time.get_ticks() - space_ability_start_time < 2000:
            for row in range(ROWS):
                for col in range(COLS):
            ...
            player_rect = player.get_rect()
            for row in range(max(0, player_rect.y // CELL_SIZE - 3), min(ROWS, player_rect.y // CELL_SIZE + 4)):
                for col in range(max(0, player_rect.x // CELL_SIZE - 3), min(COLS, player_rect.x // CELL_SIZE + 4)):
                    maze[row][col].draw(WINDOW, 0, 0)  # No camera offset
        ...
        if player.rect.collidepoint(FINISH_POINT):
            player_won = True 
            score = (frame_rem//FPS)*150 + coins_collected*200 + boosters_collected*50 + 600 
        ...
        if frame_rem < 0 :
            ...
            running = False
            score = ((frame_rem+1)*150)//FPS + coins_collected*200 + boosters_collected*50
            if space_ability_used :
                score = max(0,score-200)
