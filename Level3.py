...
def play_level3(player,WINDOW):
    ...
    boosters = []
    for j in range(NUMBER_OF_BOOSTERS_3):
    ...
    while running :
        ...
        for event in pygame.event.get() :
            ...
            if event.type == pygame.KEYDOWN :
                ...
                for booster in boosters[:] :
                    expanded_booster_rect = booster.rect.inflate(5,5) #used for better accuarcy of collision
        ...
        player_rect = player.get_rect()
        for row in range(max(0, player_rect.y // CELL_SIZE - 2), min(ROWS, player_rect.y // CELL_SIZE + 3)):
            for col in range(max(0, player_rect.x // CELL_SIZE - 2), min(COLS, player_rect.x // CELL_SIZE + 3)):
        ...
        if player.rect.collidepoint(FINISH_POINT):
            player_won = True 
            score = (frame_rem//FPS)*100 + coins_collected*200 + boosters_collected*50 + 800
        ...
        if frame_rem < 0 :
            ...
            running = False
            score = ((frame_rem+1)//FPS)*100 + coins_collected*200 + boosters_collected*50
